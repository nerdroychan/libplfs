...
#define plfs_VERSION_MAJOR 2
#define plfs_VERSION_MINOR 5
#define plfs_VERSION_PATCH 1
#define plfs_package_string "PLFS 2.5.1"
...
/*
 * plfs_filetype CONTAINER index types 
 *
 * XXX: these should be private to the Container LogicalFS... but,
 * the MPI container optimizations break layering and run before
 * the ContainerIndex does.   e.g. we may precompute an index before
 * we even call plfs open (passing the precomputed index in as an
 * arg to open).   we can only precompute BYTERANGE indexes, so we 
 * need to know if that is what we have or not before the index 
 * even runs.   so the MPI code will call plfs_get_fileinfo to get
 * the filetype and the index type.
 */
#define CI_UNKNOWN   0
